...
@cached_as(
    Activity.objects.select_related(
        'bounty', 'tip', 'tip__recipient_profile'
    ).filter(bounty__network='mainnet').order_by('-created'), timeout=120)
def get_activities(tech_stack=None, num_activities=15):
    # get activity feed

    activities = Activity.objects.select_related('bounty', 'tip', 'tip__recipient_profile') \
        .filter(bounty__network='mainnet').order_by('-created')
    if tech_stack:
